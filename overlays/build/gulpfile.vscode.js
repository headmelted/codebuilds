...
gulp.task('vscode-translations-push', ['optimize-vscode'], function() {
    ...
    ).pipe(i18n.findObsoleteResources(apiHostname, apiName, apiToken)).pipe(i18n.pushXlfFiles(apiHostname, apiName, apiToken));
});
...
gulp.task('vscode-translations-push-test', ['optimize-vscode'], function() {
    ...
    return es.merge(
        ...
        gulp.src(pathToSetup).pipe(i18n.createXlfFilesForIsl()),
        gulp.src(pathToExtensions).pipe(i18n.createXlfFilesForExtensions())
    ).pipe(i18n.findObsoleteResources(apiHostname, apiName, apiToken)).pipe(vfs.dest('../vscode-transifex-input'));
});
