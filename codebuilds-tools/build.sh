...
if [[ "${CROSS_TOOLCHAIN}" == "true" ]]; then
  ...
  echo "deb [arch=amd64,i386] http://archive.ubuntu.com/ubuntu zesty main universe multiverse restricted" | tee /etc/apt/sources.list;
  
  echo "Adding 16.04 (zesty) package sources for ${ARCH}...";
  echo "deb [arch=${ARCH}] http://ports.ubuntu.com/ubuntu-ports zesty main universe multiverse restricted" | tee -a /etc/apt/sources.list;
  
  echo "Adding 16.04 (zesty) package sources for source code...";
  echo "deb-src http://archive.ubuntu.com/ubuntu zesty main universe multiverse restricted" | tee -a /etc/apt/sources.list;
